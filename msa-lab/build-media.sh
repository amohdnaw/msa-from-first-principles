...
#   MSALAB_VOICE=1 ./build-media.sh   narrated + captions (needs network for gTTS)
#   MSALAB_VOICE=1 MSALAB_VOICE_SERVICE=recorder ./build-media.sh   your own voice
#
...
echo "voice=${MSALAB_VOICE:-0} service=${MSALAB_VOICE_SERVICE:-kokoro} quality=$QUALITY"
echo
...
for entry in "${SCENES[@]}"; do
  # Levels ship one at a time, so most scene files do not exist yet. Skipping a
  # missing one keeps `./build-media.sh` a single command from day one instead of
  # something that only works once all seven are written.
  if [ ! -f "src/msalab/${entry%%:*}.py" ]; then
    continue
  fi
  ...
  printf '=== %-14s %s\n' "$klass" "(src/msalab/$file.py)"

  PYTHONPATH=src $VENV/manim "$QUALITY" --disable_caching \
    "src/msalab/$file.py" "$klass" >/dev/null 2>&1 \
