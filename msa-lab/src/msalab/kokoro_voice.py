...
manim-voiceover the resulting wav, which keeps `msa-lab/.venv` small and makes
...
VOICE = os.environ.get("MSALAB_KOKORO_VOICE", "am_michael")
SPEED = float(os.environ.get("MSALAB_KOKORO_SPEED", "0.92"))
...
WORKER_PYTHON = Path(os.environ.get(
    "MSALAB_KOKORO_PYTHON", str(Path.home() / "tts-lab/venv/bin/python")))
    ...
    def __init__(self, voice: str = VOICE, speed: float = SPEED, **kwargs: object) -> None:
        if not WORKER_PYTHON.exists():
            raise RuntimeError(
                ...
                "Install it, or point MSALAB_KOKORO_PYTHON at one that has kokoro:\n"
