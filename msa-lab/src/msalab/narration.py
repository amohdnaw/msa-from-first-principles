...
- `MSALAB_VOICE=1` renders real speech and the block holds until the line ends.
...
Same source, same pacing, audio optional. Nothing to re-tune.
    ...
    PYTHONPATH=src .venv/bin/manim -qh src/msalab/level06_scene.py Level06

    # narrated
    MSALAB_VOICE=1 PYTHONPATH=src .venv/bin/manim -qh src/msalab/level06_scene.py Level06

Swapping the synthetic voice for a real one is a one-line change in
`_speech_service` — see MSALAB_VOICE_SERVICE below.
"""
...
def _speech_service():
    ...
    only shells out to it — see `msalab/kokoro_voice.py`. If that interpreter is
    missing, the render falls back to gTTS with a warning rather than dying,
    because a voice regression should not stop a build.

        MSALAB_VOICE=1 ./build-media.sh                      # kokoro
        MSALAB_VOICE=1 MSALAB_VOICE_SERVICE=gtts ./build-media.sh
        MSALAB_VOICE=1 MSALAB_VOICE_SERVICE=recorder ./build-media.sh   # your voice
